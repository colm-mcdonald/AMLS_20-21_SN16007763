...
class A2():
	...
	def __getData(self):
		...
		if(not os.path.isfile('a2_input.npy') or not os.path.isfile('a2_output.npy')):
			X, y = face_features.extract_features_labels("./Datasets/celeba","img")
			Y = np.array([y, -(y-1)]).T
			np.save('a2_input',X)
			np.save('a2_output',Y)
		else:
			X=np.load('a2_input.npy')
			Y=np.load('a2_output.npy')
	...
	def training_curve(self):
		...
		train_sizes, train_scores, valid_scores = learning_curve(
			svm.SVC(kernel='poly',degree=3,C=1.0,random_state=1),X.reshape((len(X), 68*2)), list(zip(*Y))[0],
